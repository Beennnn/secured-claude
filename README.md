...
| **Multi-arch image (linux/arm64)** | Kaniko builds amd64 only on the macbook-local runner ; arm64 users (Apple Silicon, AWS Graviton) must `docker run --platform linux/amd64` and pay the QEMU overhead | v0.3 — Kaniko `--customPlatform` or remote builder |
