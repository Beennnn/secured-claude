...
**v0.6.0** — every claim below is backed by an artifact you can re-run yourself.
...
#     pytest 161/161, coverage 92.6 % ; SBOM 139 packages.
...
| 37 ADRs justifying every decision | `docs/adr/0000-template.md` + `0001..0037-*.md` | `ls docs/adr/` |
...
> - 🏛 **Architecture** — Hexagonal-lite Python broker (host) + Cerbos PDP (container) + Claude Code CLI (container) ; clear trust boundary between intent (LLM) and execution (broker). 37 ADRs covering every security + operational decision.
> - ✅ **Qualité** — 161 unit + integration tests, 92.6 % coverage gate. Security audit demonstration with 19 red-team scenarios + 7 happy-paths + policy fuzz + 8 static scans, run on every release.
...
> - 🛠 **DevX** — `secured-claude` CLI feels like `claude` (TTY preserved) ; `audit` subcommand surfaces the evolving allowlist ; `doctor` validates the install end-to-end ; principal directory pluggable via env (`SECURED_CLAUDE_PRINCIPALS` for YAML, `SECURED_CLAUDE_IDP_URL` + `SECURED_CLAUDE_IDP_BEARER_TOKEN` + `SECURED_CLAUDE_IDP_CACHE_TTL_S` for HTTP IdP).
