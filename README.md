...
**v0.7.2** — every claim below is backed by an artifact you can re-run yourself.
...
#     pytest 237/237, coverage 91.2 % ; SBOM 140 packages.
...
| 42 ADRs justifying every decision | `docs/adr/0000-template.md` + `0001..0042-*.md` | `ls docs/adr/` |
...
> - 🏛 **Architecture** — Hexagonal-lite Python broker (host) + Cerbos PDP (container) + Claude Code CLI (container) ; clear trust boundary between intent (LLM) and execution (broker). 42 ADRs covering every security + operational decision.
> - 📊 **Observabilité** — Prometheus counters at `/metrics` (loopback-only) covering JWT-deny / JWKS-degraded / stale-cache / cerbos-unavailable / multi-issuer routing-misses (ADR-0042). Standard Grafana alert recipes documented in the ADR.
> - ✅ **Qualité** — 237 unit + integration tests, 90 % coverage gate. Security audit demonstration with 19 red-team scenarios + 7 happy-paths + policy fuzz + 8 static scans, run on every release.
